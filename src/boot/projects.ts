...
export default boot(() => {
  ...
  const modules = import.meta.glob('/public/markdown/*/project.json')
  for (const path in modules) {
    ...
    modules[path]().then(module => {
      ...
      module.versions.forEach(v => {
        promises = promises.concat(import('/public/markdown/' + module.key.toLowerCase() + '/' + v + '/pages.json').then(value => {
